...
class VOT(object):
  ...
  This class is only a wrapper around the TraX protocol and can be used for
  single or multi-object tracking. The wrapper assumes that the experiment will
  provide new objects onlf at the first frame and will fail otherwise.
  ...
  def __init__(self, region_format, channels=None, multiobject: bool = None):
    ...
    """
    assert (region_format in [trax.Region.RECTANGLE,
                              trax.Region.POLYGON,
                              trax.Region.MASK])
    ...
    self._trax = trax.Server(
        region_formats=[region_format],
        image_formats=[trax.Image.PATH],
    ...
    assert (len(request.objects) > 0 and
            (multiobject or len(request.objects) == 1))
  ...
  def region(self):
      """Returns initialization region for the first frame in single object
      tracking mode.
  ...
  def objects(self):
    """Returns initialization regions for the first frame in multi object
    tracking mode.
  ...
  def report(self, status, confidence=None):
    ...
    Arguments:
        status: region for the frame or a list of regions in case of multi
            object tracking
        confidence: confidence for the object detection, used only in single
    ...
    if confidence is not None and not self._multiobject:
...
class VOTManager(object):
  """VOT Manager is provides a simple interface for running multiple single
  object trackers in parallel. Trackers should implement a factory interface.
  """

  def __init__(self, factory, region_format, channels=None):
    """Constructor for the manager.
    The factory should be a callable that accepts two arguments: image and
    region and returns a callable that accepts a single argument (image)
  ...
  def run(self):
    """Run the tracker, the tracking loop is implemented in this function, so
    it will block until the client terminates the connection.
    """
