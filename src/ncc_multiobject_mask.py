...
class NCCTracker(object):
  ...
  def track(self, image):
    ...
    width = right - left
    height = bottom - top
    if width < self.template.shape[1] or height < self.template.shape[0]:
    ...
    rect = [left + max_loc[0], top + max_loc[1], self.size[0], self.size[1]]
    output_size = (image.shape[1], image.shape[0])
    return self._mask_from_rect(rect, output_size)
